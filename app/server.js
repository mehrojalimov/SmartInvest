  ...
  *****************************************************************************************************************/
  ...
  app.get("/api/stock/:symbol", async (req, res) => {
    const stockSymbol = req.params.symbol.toUpperCase().replace("\n", "");  // Get the symbol from the URL parameter
    ...
    try {
      ...
      return res.status(500).json({ error: (`An error occurred while fetching stock data: ${error.message}`) });
    }
  });
  ...
  //     // Insert into database (ensure stock_name exists in stocks table)
  //     const result = await pool.query(
  //       `INSERT INTO portfolio (user_id, stock_id, transaction_type, quantity)
  //       VALUES ($1, (SELECT stock_id FROM stocks WHERE stock_name = $2), $3, $4) RETURNING *`,
  //       [1, stock_name, transaction_type, quantity] // Replace '1' with the actual user_id
  //     );

  //     res.status(200).json({ message: "Transaction successful", data: result.rows[0] });
  //   } catch (error) {
